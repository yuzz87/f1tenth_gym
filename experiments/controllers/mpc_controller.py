...
class MPCController(BaseController):
    """Minimal kinematic MPC for centerline tracking."""
    ...
    def __init__(self, conf, car_params):
        ...
        self.horizon = int(self.controller_conf.get("horizon", 8))
        self.maxiter = int(self.controller_conf.get("maxiter", 25))
        self.steer_weight = float(self.controller_conf.get("steer_weight", 0.05))
        self.steer_delta_weight = float(self.controller_conf.get("steer_delta_weight", 1.0))
        ...
        self.nominal_steer_weight = float(self.controller_conf.get("nominal_steer_weight", 2.5))
        self.prev_solution = np.zeros((self.horizon,), dtype=float)
    ...
    def plan(self, obs):
        ...
        initial_guess = np.clip(
            0.6 * self.prev_solution + 0.4 * nominal_sequence,
            self.steer_min,
            self.steer_max,
        )
        result = minimize(
            fun=self._objective,
            x0=initial_guess,
            args=(state, speed_target, reference, nominal_sequence),
            method="L-BFGS-B",
            bounds=[(self.steer_min, self.steer_max)] * self.horizon,
            options={"maxiter": self.maxiter},
        )
        if result.success:
            steer_sequence = np.asarray(result.x, dtype=float)
        else:
            steer_sequence = initial_guess

        self.prev_solution = np.empty_like(steer_sequence)
        self.prev_solution[:-1] = steer_sequence[1:]
        self.prev_solution[-1] = steer_sequence[-1]

        steer = float(np.clip(steer_sequence[0], self.steer_min, self.steer_max))
    ...
    def _objective(self, steer_sequence, state, speed_target, reference, nominal_sequence):
        ...
        for index in range(self.horizon):
            ...
            cost += self.steer_weight * steer * steer
            cost += self.steer_delta_weight * delta * delta
            cost += self.nominal_steer_weight * (steer - nominal_sequence[index]) ** 2
            prev_steer = steer
        return float(cost)
